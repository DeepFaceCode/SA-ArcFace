...
class IResNet(nn.Module):
    ...
    def __init__(self,
        ...
        global bes
        bes = 512 * block.expansion * self.fc_scale
        global bes1
        bes1 = 512 * block.expansion
...
class Self_Atten(torch.nn.Module):
    ...
    def forward(self,inputs_x):
        with torch.cuda.amp.autocast(self.fp16):
            ...
            proj_query = self.query_conv(self.x).view(m_batchsize, -1, width * height).permute(0, 2, 1)  # B X CX(N)
            proj_key = self.key_conv(self.x).view(m_batchsize, -1, width * height)  # B X C x (*W*H)
            if self.fp16:
                proj_query.float()
                proj_key.float()
            ...
            proj_value = self.value_conv(self.x).view(m_batchsize, -1, width * height)  # B X C X N
