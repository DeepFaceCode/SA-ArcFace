...
from backbones.iresnet import Self_Atten, ConcatNet_
...
def main(args):
    ...
    backbone_Resnet = get_model(
        cfg.network, dropout=0.0, fp16=cfg.fp16, num_features=cfg.embedding_size).cuda()
    backbone_Sa = Self_Atten(fp16=cfg.fp16,).cuda()
    backbone = ConcatNet_(backbone_Resnet, backbone_Sa,cfg.is_samodel).cuda()
    ...
    for epoch in range(start_epoch, cfg.num_epoch):
        ...
        for _, (img, local_labels) in enumerate(train_loader):
            ...
            with torch.no_grad():
                ...
                callback_logging(global_step, loss_am, epoch, cfg.fp16, lr_scheduler.get_last_lr()[0], amp,gamma=iresnet.gamma_g)
        ...
        if rank == 0:
            path_module = os.path.join(cfg.output, "model0.pt")
